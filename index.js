...
firebase.initializeApp({
    ...
    databaseURL: "https://mythical-mason-145813-default-rtdb.asia-southeast1.firebasedatabase.app"
});
...
Scraper.prototype.start = async function(searchString, matchCase, capabilities) {
    return await this.strategy.execute(searchString, matchCase, capabilities);
};
var ScrapingStrategy = async function(searchString, matchCase, capabilities) {};
ScrapingStrategy.prototype = Object.create(Strategy.prototype);
ScrapingStrategy.prototype.execute = async function(searchString, matchCase, capabilities) {
...
AmazonScrapStrategy.prototype.scrap = async function(searchString, matchCase, capabilities) {
    driver = new webdriver.Builder()
        .usingServer('http://192.168.1.108:4000/wd/hub')
        .withCapabilities(capabilities)
        .build();
    await driver.get("https://www.amazon.com");
    var searchBox = await driver.findElement(webdriver.By.id("twotabsearchtextbox"));
    await searchBox.sendKeys(searchString, webdriver.Key.ENTER);
    var searchResults = await driver.findElements(webdriver.By.css("div[data-asin]"));
    ...
    {
        try {
            var result = await searchResult.findElement(webdriver.By.className(`a-link-normal a-text-normal`));
            ...
            {
                amazonArray.push({store: "Amazon", productName: productName, productLink: productLink});
...
EBayScrapStrategy.prototype.scrap = async function(searchString, matchCase, capabilities) {
    var driver = new webdriver.Builder()
        .usingServer('http://192.168.1.108:4000/wd/hub')
        .withCapabilities(capabilities)
        .build();
    ...
    await searchBox.sendKeys(searchString, webdriver.Key.ENTER);
    var searchResults = await driver.findElements(webdriver.By.className("s-item__wrapper clearfix"));
    ...
    {
        try {
            var element = await searchResult.findElement(webdriver.By.className(`s-item__link`));
            var productLink = await element.getAttribute("href");
            var productName = (await element.getText()).toString();
            if (productName.toLowerCase().includes(matchCase))
            {
                eBayArray.push({store: "eBay", productName: productName, productLink: productLink});
